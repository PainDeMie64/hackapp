...
export async function runFullPipeline(config: PipelineConfig): Promise<PipelineResult> {
	const { db, storage, bedrock } = config;
	...
	const scrapeSettled = await Promise.allSettled(
		allSources.slice(0, 20).map(async (source) => {
			try {
				...
				errors.push({ step: 'scrape', entity: source.url, error: e instanceof Error ? e.message : String(e) });
				return null;
			}
		})
	);

	const scraped = scrapeSettled
		.filter((r): r is PromiseFulfilledResult<NonNullable<Awaited<ReturnType<typeof scrapeUrl>>>> =>
			r.status === 'fulfilled' && r.value !== null)
		.map(r => r.value);

	// ── Phase 2: Extract companies from scraped text ────────────────────
	type ExtractedCompanyWithId = { companyId: string; name: string; text: string; sourceId: string };
	const extractedCompanies: ExtractedCompanyWithId[] = [];

	await Promise.allSettled(
		scraped.filter(s => s.extractedTextR2Key && s.statusCode === 200).map(async (scrape) => {
			await sem.acquire();
			try {
				...
				for (const company of extraction.companies) {
					try {
						...
						errors.push({ step: 'upsert_company', entity: company.name, error: e instanceof Error ? e.message : String(e) });
					}
				}
				...
				errors.push({ step: 'extract_company', entity: scrape.url, error: e instanceof Error ? e.message : String(e) });
			} finally {
				sem.release();
			}
		})
	...
	// ── Phase 3: News + Scores in parallel per company ──────────────────
	await Promise.allSettled(
		extractedCompanies.map(async ({ companyId, name, text, sourceId }) => {
			...
			await Promise.allSettled([
				...
				(async () => {
					...
					try {
						...
						errors.push({ step: 'extract_news', entity: name, error: e instanceof Error ? e.message : String(e) });
					} finally {
						sem.release();
					}
				})(),

				// Prospect scoring (50 criteria)
				(async () => {
					await sem.acquire();
					try {
						const result = await bedrock.converse(
						...
						errors.push({ step: 'score_prospect', entity: name, error: e instanceof Error ? e.message : String(e) });
					} finally {
						sem.release();
					}
				})(),
